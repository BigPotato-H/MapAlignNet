...
def read_feather(path: Path, columns: Optional[Tuple[str, ...]] = None) -> pd.DataFrame:
    ...
    #d: pd.DataFrame = feather.read_table(path, columns=columns)
    return data
...
def read_lidar_sweep(fpath: Path, attrib_spec: str = "xyz") -> NDArrayFloat:
    ...
    #sweep_df = data: pd.DataFrame = feather.read_feather(fpath, columns=columns)
    sweep_df: pd.DataFrame = feather.read_feather(fpath)
