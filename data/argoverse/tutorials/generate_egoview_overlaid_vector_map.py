...
def generate_egoview_overlaid_map(
    ...
    for _, cam_enum in enumerate(cam_names):
        ...
        for i, img_fpath in enumerate(cam_im_fpaths):
            ...
            #if cam_timestamp_ns != 315967989399927216:
                #continue
...
def render_egoview(
    ...
    """
    #colorful map on image
    #save_dir = output_dir / log_id /"ego_map"
    #img_fname = f"{egoview_renderer.pinhole_cam.cam_name}_{cam_timestamp_ns}_vectormap.jpg"


    #mask label
    save_dir = output_dir / log_id / "label"
    #img_fname = f"{egoview_renderer.pinhole_cam.cam_name}_{cam_timestamp_ns}.png"
    img_fname = f"{cam_timestamp_ns}.png"
    ...
    img_rgb: NDArrayByte = io_utils.read_img(img_fpath, "BGR")
    ...
    img_empty: NDArrayByte = np.full((img_h, img_w, 3), fill_value=0, dtype=np.uint8)

    #copy original image
    img_empty = img_rgb.copy()
    ...
    if dump_single_frames:
        ...
        io_utils.write_img(save_fpath, frame_rgb, channel_order="BGR")
        #t_fname = f"{egoview_renderer.pinhole_cam.cam_name}_{cam_timestamp_ns}_sat.png"
        #io_utils.write_img(save_dir / t_fname, img_rgb, channel_order="BGR")
    return frame_rgb
...
def render_egoview_with_occlusion_checks(
    ...
    #img_rgb: NDArrayByte = img_canvas
    return img_rgb
