...
def train(model,
    ...
    for i, data_item in enumerate(prog_bar):
        ...
        map, img, img_label, gt_Rt, K, upp = data_item
        map, img, img_label, gt_Rt, K, upp = \
            map.to(device), img.to(device), img_label.to(device), \
            gt_Rt.to(device), K.to(device), upp.to(device)

        optimizer.zero_grad()
        if opt.AddAttention:
            outputs = model(img, upp)
        else:
            outputs = model(img)

        ##### BATCH-WISE LOSS #####
        loss_seg = criterion(outputs, img_label.long())
        #reprojection_loss, pose_difference = run(map, outputs, gt_Rt, K)

        #lambda_seg = torch.exp(log_lambda_seg)
        #lambda_pose_proj = torch.exp(log_lambda_pose_proj)
        ...
        # For pixel accuracy.
        labeled, correct = pix_acc(img_label, outputs, num_classes)
