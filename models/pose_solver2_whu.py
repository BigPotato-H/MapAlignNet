...
import cv2
import os
from data.whu_data_loader import pose_6dof_to_matrix, camera_to_pano
...
def get_view_hd_img(hd, img,P):
    B, H, W = img.shape[0], img.shape[1], img.shape[2]

    batch_proj_img = torch.zeros(B, H, W, dtype=torch.float32).cuda()
    for bi in range(B):
        pose = P[bi].detach().cpu().numpy()
        t,r = pose_6dof_to_matrix(pose)
        ego_points = hd[bi].detach().cpu().numpy()
        cam_points = r @ ego_points + t[:, np.newaxis]
        img_points = camera_to_pano(cam_points.T, scale=0.25)
        img_points = np.array(img_points).T
        if len(img_points.shape) != 2:
            continue
        proj_xrev = torch.from_numpy(img_points[0, :].round()).long()
        proj_yrev = torch.from_numpy(img_points[1, :].round()).long()
        proj_xrev = proj_xrev.clamp(0, W - 1)
        proj_yrev = proj_yrev.clamp(0, H - 1)
        batch_proj_img[bi, proj_yrev, proj_xrev] = 1
