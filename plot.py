...
def plot_mean_compare():
    ...
    print(pivot_df)
    pivot_df.to_csv(out_dir + 'overall_compare.csv', float_format="%.2f", encoding='utf-8')
    ...
    print(pivot_rt_df)
    pivot_rt_df.to_csv(out_dir + '6dof_compare.csv', float_format="%.2f", encoding='utf-8')
    ...
    df_melted = pivot_plot_df.melt(id_vars='Model', value_vars=['iou'],
                               var_name='Metric', value_name='Value')
    sns.barplot(data=df_melted, x='Model', y='Value', hue='Metric')


    # Set plot title and labels
    #plt.title('PixAcc and IoU for Different Models')
...
def plot_rt_and_recall():
    ...
    for model_name in model_names:
        ...
        for i in range(0, len(t_thresholds)):
            #recall = ((err_t < t_thresholds[i]) & (err_r < r_thresholds[i])).any().mean()  # Proportion of instances with MTE < threshold
    ...
    plt.xlabel('MTE threshold (m)')
    plt.ylabel('Recall(%)')
    #plt.title('MTE vs. Recall')
    plt.grid(True)
    plt.legend()
    plt.savefig(out_dir + 'MTE_recall.png', dpi=DPI)
    plt.close()

    plt.figure(figsize=(8, 6))
    for model_name, model_data in all_data.items():
        plt.plot(model_data['err_r'], model_data['recall_r'], label=model_name)
    plt.xlabel('MRE threshold (deg)')
    plt.ylabel('Recall(%)')
    #plt.title('MRE vs. Recall')
    plt.grid(True)
    plt.legend()
    plt.savefig(out_dir + 'MRE_recall.png', dpi=DPI)
...
def print_recall():
    ...
    print(all_data.T)
    all_data.T.to_csv(out_dir + 'recall.csv', float_format="%.2f", encoding='utf-8')
